...
Client.prototype.sendProduceRequest = function (payloads, requireAcks, ackTimeoutMs, cb) {
  ...
  function buildRequest (payload, cb) {
    ...
    var timestamp = payload.messages[0].timestamp;
    var timestampType = payload.messages[0].timestampType;
    var magic = payload.messages[0].magic;
    ...
    codec.encode(innerSet, function (err, message) {
      if (err) return cb(err);
      payload.messages = [ new Message(magic, attributes, timestamp, timestampType, '', message) ];
      cb();
    });
  }
};
