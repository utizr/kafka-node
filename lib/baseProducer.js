...
BaseProducer.prototype.buildPayloads = function (payloads, topicMetadata) {
  ...
  payloads.forEach((p) => {
    ...
    p.magic = p.hasOwnProperty('timestamp') ? 1 : 0;
    p.timestamp = p.hasOwnProperty('timestamp') ? p.timestamp : 0;
    p.timestampType = p.hasOwnProperty('timestampType') ? p.timestampType : 0;
    let messages = _.isArray(p.messages) ? p.messages : [p.messages];

    messages = messages.map(function (message) {
      if (message instanceof KeyedMessage) {
        return new Message(p.magic, 0, p.timestamp, p.timestampType, message.key, message.value);
      }
      return new Message(p.magic, 0, p.timestamp, p.timestampType, '', message);
    });
